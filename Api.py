...
async def settings(selected_option: str = Form(...), date: list = Form(...), market: list[float] = Form(...), load: list[float] = Form(...)):
    # Converter a string de data em um objeto datetime


    a = selected_option

    table_data = pd.DataFrame({'date': date, 'market': market, 'load': load}, index=None)
    ...
    for day in GeneralSettings.all_days:
            ...
            main.first_dt_text = dt.datetime.strftime(first_dt, '%Y-%m-%d %H:%M:%S')
